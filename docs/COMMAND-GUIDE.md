...
| `/create-execution-plan` | Write a detailed execution plan to files for a future session | `PROJECT-CONTEXT.md` filled in |
...
This reads your `PROJECT-CONTEXT.md` and produces every artifact in dependency order. Use this when you want the complete set without manual steps.
...
| `_plan/TASK-MANIFEST.json` | Machine-readable JSON of all 16 tasks with dependencies |
| `_plan/RUNSHEET.md` | Operator checklist — work through tasks ticking off criteria |

**To execute in a future session:**

```
Read _plan/EXECUTION-PLAN.md and execute all tasks in order
```
...
**Provide data.** The more quantitative data in `PROJECT-CONTEXT.md`, the more specific the artifacts. Vague inputs produce generic outputs.
